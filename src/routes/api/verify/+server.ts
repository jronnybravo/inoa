...
export const POST: RequestHandler = async ({ request }) => {
    ...
    if (!runId || !code) {
        error(400, 'Missing runId or code');
    }
    ...
    if (!record) {
        error(404, 'No verification pending for this request');
    }
    if (record.consumedAt) {
        error(400, 'That code was already used');
    }
    if (record.expiresAt < new Date()) {
        error(400, 'That code has expired');
    }
    // Bounded so six digits cannot be walked through.
    if (record.attempts >= MAX_ATTEMPTS) {
        error(429, 'Too many attempts; start a new request');
    }
